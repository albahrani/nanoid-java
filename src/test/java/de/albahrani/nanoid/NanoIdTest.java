...
/**
 * Comprehensive test suite for NanoId covering functionality, performance, thread safety, distribution, and error handling.
...
public class NanoIdTest {
    ...
    private static final int DEFAULT_ID_LENGTH = 21;
    private static final int LARGE_ID_COUNT = 100_000;
    private static final int PERFORMANCE_ITERATIONS = 100_000;
    private static final int MEMORY_LIMIT_MB = 50;
    private static final int THREAD_COUNT = 10;
    private static final int IDS_PER_THREAD = 10_000;

    private static void assertAllCharsInAlphabet(String id, String alphabet) {
        for (char c : id.toCharArray()) {
            assertTrue(alphabet.indexOf(c) >= 0, "Character '" + c + "' should be in alphabet");
        }
    }
    ...
    class DefaultNanoIdTests {
        ...
        void generatesDefaultNanoId() {
            ...
            assertEquals(DEFAULT_ID_LENGTH, id.length(), "Default ID should be 21 characters");
            assertTrue(URL_ALPHABET_PATTERN.matcher(id).matches(), "ID should only contain URL alphabet characters");
        }

        @Test
        void generatesCustomSizeNanoId() {
    ...
    public void testUniqueness() {
        ...
        assertEquals(LARGE_ID_COUNT, ids.size(), "Should have generated exactly " + LARGE_ID_COUNT + " unique IDs");
    }

    @Test
    public void testDistributionUniformity() {
        String alphabet = "abc";
        int iterations = 30_000; // Should be multiple of alphabet.length()
    ...
    public void testPerformanceOptimization() {
        ...
        System.out.printf("Generated %d nanoids in %.2f ms (%.2f ops/ms)%n", 
                         PERFORMANCE_ITERATIONS, durationMs, PERFORMANCE_ITERATIONS / durationMs);
